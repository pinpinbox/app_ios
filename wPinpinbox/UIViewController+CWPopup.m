...
//  check if current app orientation is portrait/portrait upsidedown
...
- (UIImage *)getScreenImage {
    ...
    if ([UIViewController isOrientationPortrait]){
...
- (void)wupresentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        if (self.useBlurForPopup) {
            ...
            fadeView.alpha = 0.0f;
            [self.view addSubview:fadeView];
            UIButton *btn=[wTools W_Button:self frame:CGRectMake(0, 0,fadeView.bounds.size.width, fadeView.bounds.size.height) imgname:@"" SELL:@selector(wubtn:) tag:1];
            [fadeView addSubview:btn];
...
- (void)wu2presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        self.popupViewController.view.autoresizingMask = UIViewAutoresizingNone;
        [self.popupViewController viewWillAppear:YES];
        CGRect finalFrame = [self w2getPopupFrameForViewController:viewControllerToPresent];
        // parallax setup if iOS7+
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            UIInterpolatingMotionEffect *interpolationHorizontal = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis];
        ...
        if (self.useBlurForPopup) {
            ...
            fadeView.alpha = 0.0f;
            [self.view addSubview:fadeView];
            UIButton *btn=[wTools W_Button:self frame:CGRectMake(0, 0,fadeView.bounds.size.width, fadeView.bounds.size.height) imgname:@"" SELL:@selector(wu2btn:) tag:1];
            [fadeView addSubview:btn];
        ...
        // setup
        if (flag) { // animate
            CGRect initialFrame = CGRectMake(finalFrame.origin.x, - viewControllerToPresent.view.frame.size.height, finalFrame.size.width, finalFrame.size.height);
            viewControllerToPresent.view.frame = initialFrame;
            [self.view addSubview:viewControllerToPresent.view];
            [UIView animateWithDuration:ANIMATION_TIME delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                viewControllerToPresent.view.frame = finalFrame;
...
-(void)wubtn:(id)sender{
    [self dismissPopupViewControllerAnimated:YES completion:^(void){}];
}
-(void)wu2btn:(id)sender{
    [self w2dismissPopupViewControllerAnimated:YES completion:^(void){}];
}
- (void)wpresentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        ...
        self.popupViewController.view.autoresizingMask = UIViewAutoresizingNone;
        [self.popupViewController viewWillAppear:YES];
        CGRect finalFrame = [self wgetPopupFrameForViewController:viewControllerToPresent];
        // parallax setup if iOS7+
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            UIInterpolatingMotionEffect *interpolationHorizontal = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis];
...
- (CGRect)wgetPopupFrameForViewController:(UIViewController *)viewController {
    CGRect frame = viewController.view.frame;
    CGFloat x;
    CGFloat y;
    if ([UIViewController isOrientationPortrait]){
    //if (UIDeviceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        x = ([UIScreen mainScreen].bounds.size.width - frame.size.width)/2;
        y = ([UIScreen mainScreen].bounds.size.height - frame.size.height);
    } else {
        x = ([UIScreen mainScreen].bounds.size.height - frame.size.width)/2;
        y = ([UIScreen mainScreen].bounds.size.width - frame.size.height);
    }
    return CGRectMake(x, y, frame.size.width, frame.size.height);
}
- (CGRect)w2getPopupFrameForViewController:(UIViewController *)viewController {
    CGRect frame = viewController.view.frame;
    CGFloat x;
    CGFloat y;
    if ([UIViewController isOrientationPortrait]){
